...
The quickest way to open the GitHub Copilot app from a terminal is the `copilot app` command. Run it from any directory to open the app in the context of that folder:

```bash
copilot app
```

This is especially convenient when you're already working in a project directory in the Copilot CLI and want to switch to the app's graphical interface without navigating manually.

### Launching Sessions from the Terminal with Deep Links

The GitHub Copilot app also supports URL deep links for more targeted navigation. This is useful when you want to open a specific session, repository, or work item directly from scripts or terminal aliases.
