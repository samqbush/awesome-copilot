...
If you're working in the Copilot CLI and want to continue your current session in the Copilot app's graphical interface, use the `/app` command (v1.0.79+, requires GitHub Copilot app 1.1.3 or later):

```
/app
```

This opens your current CLI session in the Copilot app, picking up exactly where you left off — with the same conversation context and working directory. This is useful when you've started a task in the terminal and want to switch to the app's visual interface for reviewing changes or managing parallel work.
