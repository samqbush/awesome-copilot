...
> **Auto-updates (v1.0.78+)**: First-party plugins (those installed from the official marketplace) are automatically updated to the latest version at session start. This means you always have the newest features and fixes without running a separate update command.
