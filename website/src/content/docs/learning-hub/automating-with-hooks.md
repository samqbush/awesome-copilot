...
> **Exit code 2 for explicit denial (v1.0.70+)**: `preToolUse` hooks that exit with **code 2** are treated as an explicit denial. This distinction lets you differentiate between a hook script that failed unexpectedly (any other non-zero code) and one that deliberately blocked the tool call. Exit 0 to approve, exit 2 to deny, and exit with another code for unexpected failures.
