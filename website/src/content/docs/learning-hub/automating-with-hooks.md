...
| Exit code | Behaviour |
|-----------|-----------|
| `0` | **Approve** — the tool execution proceeds normally |
| `2` | **Deny** — the tool call is blocked (v1.0.70+) |
| any other non-zero | **Error** — treated as a hook failure; the tool call may or may not proceed depending on the error policy |

> **Tip (v1.0.70+)**: Use exit code `2` to explicitly deny a tool call. This is the recommended way to block tool executions, as it distinguishes a deliberate denial from an unexpected hook error.
...
- **Use exit code 2 to deny in `preToolUse`**: Exit code `2` explicitly denies a tool call (v1.0.70+). For other hook types, any non-zero exit blocks the triggering action. Use this for must-pass checks.
