...
| `agentStop` | Main agent finishes responding to a prompt | Run final linters/formatters, validate complete changes. Receives `stop_hook_active: true` when the CLI forces continuation after 8 consecutive blocks. |
...
> **Loop protection (v1.0.72+)**: If an `agentStop` hook consistently blocks (non-zero exit), the CLI ends the turn after 8 consecutive blocks to prevent infinite loops. On a forced continuation, the hook receives a `stop_hook_active: true` field in its JSON input — use this to detect the forced state and gracefully self-limit:
>
> ```bash
> #!/usr/bin/env bash
> # Self-limit when the CLI forces a continuation
> if echo "$HOOK_INPUT" | jq -e '.stop_hook_active == true' > /dev/null 2>&1; then
>   echo "Forced continuation detected — skipping lint check" >&2
>   exit 0
> fi
