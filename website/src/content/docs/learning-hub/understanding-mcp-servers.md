...
- **OAuth**: MCP servers can use the OAuth flow to authenticate with external services. The CLI handles the browser redirect and token storage automatically. This also works when running in ACP (Agent Coordination Protocol) mode. As of v1.0.77+, the browser-based OAuth flow is also the **default for `copilot login`** on local interactive terminals; device code remains the default for remote and headless environments. Use `--web-flow` or `--device-code` to force a specific flow, or pick one in the interactive `/login` command.
