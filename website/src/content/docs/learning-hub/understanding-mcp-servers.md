...
**Deleting MCP servers** (v1.0.72+): When you delete an MCP server with `/mcp delete`, the CLI **stops the server's running background process** immediately — you don't need to restart the session for the change to take effect. Similarly, toggling `/sandbox` restarts only local MCP servers, leaving remote MCP servers connected.
