...
> **Canvas support in Copilot CLI (v1.0.71+)**: Canvas extensions are now also available in the GitHub Copilot CLI. When an extension provides a canvas, the CLI supports the same extension-driven interactions, making canvas-based workflows accessible from the terminal in addition to the Copilot app.
