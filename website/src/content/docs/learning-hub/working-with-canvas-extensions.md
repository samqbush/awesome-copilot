...
> **Canvas support in Copilot CLI (v1.0.71+)**: Canvas extensions are no longer limited to the Copilot app. The CLI now supports canvas interactions for extension-driven workflows, so extensions that use `createCanvas` capabilities can be driven from both the app and terminal sessions.
