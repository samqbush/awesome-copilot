...
**New models (v1.0.74–v1.0.75)**: Recent releases added support for **Gemini 3.6 Flash** (v1.0.74) and **Claude Opus 5** (v1.0.75). These join the existing lineup of Anthropic, Google, and OpenAI models available in the picker.

**Plan-mode model selection** (v1.0.74+): Use `/model plan` (or `/model --plan`) to pick a separate model specifically for plan mode. Pass a model ID to set it, `off` to clear it, or omit the ID to open the interactive picker. The plan-mode model reverts to your session model automatically when you leave plan mode:
...
This is useful when you want a more capable model for thorough planning while using a faster model for execution.

**Per-session model override** (v1.0.72+): Use `/model --session` (or `/model -s`) to change the model, reasoning effort, or context window for just the current session, leaving your global settings unchanged.
