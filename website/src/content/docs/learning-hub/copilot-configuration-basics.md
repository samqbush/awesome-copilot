...
**Plan mode model** (v1.0.74+): Use `/model plan` (or `/model --plan`) to select a separate model used only while in plan mode. Pass a model ID to set it, `off` to clear it, or no argument to open the picker. The plan-mode model reverts to your session model when you leave plan mode — useful for running a lighter/faster model during planning while switching to a more capable model for execution:

```
/model plan                  # open model picker scoped to plan mode
/model plan claude-haiku-4   # set plan mode model explicitly
