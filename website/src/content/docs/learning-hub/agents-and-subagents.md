...
> **Default depth reduced to 4** *(v1.0.71+)*: The default maximum sub-agent nesting depth was lowered from 6 to 4 to prevent runaway recursive sub-agent delegation. If your workflow needs deeper nesting, usage-based billing users can raise `subagents.maxDepth` up to 128 via `/settings`.
