...
> **Default max depth (v1.0.71+)**: The default maximum subagent nesting depth was lowered from 6 to 4. Usage-based billing users can adjust the `subagents.maxDepth` setting (up to 128) from `/settings` if their workflows require deeper delegation chains.
