...
> **Default depth reduced (v1.0.71+)**: The default maximum sub-agent nesting depth was lowered from 6 to 4 to curb runaway recursive sub-agent delegation. Usage-based billing users can still increase this by adjusting `subagents.maxDepth` in `/settings` (up to 128).
