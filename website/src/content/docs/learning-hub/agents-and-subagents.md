...
> **Default depth change (v1.0.71+)**: The default maximum subagent nesting depth was lowered from 6 to 4 to prevent runaway recursive delegation. Users on usage-based billing plans can still adjust `subagents.maxDepth` (up to 128) in `/settings`.

**Can I send follow-up messages to a running subagent?**

Yes *(v1.0.72+)*. Multi-turn subagents are always enabled — you can send follow-up messages to a running subagent without waiting for it to complete. This makes it easier to steer long-running background tasks mid-execution.
